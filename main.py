...
class GameEngine:
    ...
    def _do_green(self, frame, overlay, pose):
        ...
        self.ui.draw_game_screen(
            overlay,
            state="GREEN",
            players_alive=pose.get("players_alive", 0),
            elapsed=elapsed,
            ...
            for box, trk_id in zip(boxes.xyxy, boxes.id):
                ...
                if tid not in self._baseline_positions:
                    ...
                    if dist > MOTION_THRESHOLD and self._in_state() >= TURN_SETTLE_TIME:
                        print(f"[Elimination] Player {tid} moved {int(dist)} pixels!")
                        self.camera.capture_moment("CAUGHT!")
                        self._do_eliminate(frame, tid, (cx, cy))
                        return
    ...
    def _do_eliminate(self, frame, player_id, pos):
    ...
    def _do_elimination(self, frame, overlay):
        self.ui.draw_elimination(overlay, label=getattr(self, "_last_elim", None).label)
