...
import pygame
from config import (
    FPS_CAP, GREEN_LIGHT_MIN, GREEN_LIGHT_MAX, RED_LIGHT_MIN, RED_LIGHT_MAX,
    TURN_SETTLE_TIME, GRACE_PERIOD, MOTION_THRESHOLD, CAUGHT_HOLD
)
...
class GameEngine:
    def __init__(self, camera: Camera, servo: ServoController, laser: LaserBreakBeam, 
                 vision: ProPoseTracker, audio: AudioManager, ui: UIRenderer):
        self.camera = camera
        self.servo  = servo
        self.laser  = laser
        self.vision = vision
    ...
    def _dispatch(self, frame, overlay, pose):
        s = self._state
        if   s == State.START:          self._do_start(frame, overlay, pose)
        elif s == State.COUNTDOWN:      self._do_countdown(frame, overlay)
        elif s == State.GREEN:          self._do_green(frame, overlay, pose)
        elif s == State.TURNING_RED:    self._do_turning_red(frame, overlay, pose)
        elif s == State.RED:            self._do_red(frame, overlay, pose)
        elif s == State.CAUGHT_PAUSE:   self._do_caught_pause(frame, overlay)
        elif s == State.TURNING_GREEN:  self._do_turning_green(frame, overlay, pose)
        elif s == State.WINNER:         self._do_winner(frame, overlay, pose)
        elif s == State.RESET:          self._do_reset()
    ...
    def _do_green(self, frame, overlay, pose):
        ...
        tracked = pose.get("players_alive", 0) if pose else 0
        self.ui.draw_game_screen(overlay, "GREEN", tracked, elapsed, self._round)
    ...
    def _do_turning_red(self, frame, overlay, pose):
        tracked = pose.get("players_alive", 0) if pose else 0
        elapsed = time.time() - self._game_start
        self.ui.draw_game_screen(overlay, "TURNING", tracked, elapsed, self._round)
    ...
    def _do_red(self, frame, overlay, pose):
        tracked = pose.get("players_alive", 0) if pose else 0
        elapsed = time.time() - self._game_start
        self.ui.draw_game_screen(overlay, "RED", tracked, elapsed, self._round)
            ...
            for box, trk_id in zip(boxes.xyxy, boxes.id):
                ...
                if tid not in self._baseline_positions:
                    ...
                    if dist > MOTION_THRESHOLD and self._in_state() >= TURN_SETTLE_TIME:
                        ...
                        print(f"[Caught] {player_label} (ID {tid}) moved {int(dist)} pixels!")
                        
                        self._do_caught(player_label)
                        return
    ...
    def _do_caught(self, label):
        self._last_caught_label = label
        self.audio.announce_caught(label)
        self._go(State.CAUGHT_PAUSE)

    def _do_caught_pause(self, frame, overlay):
        self.ui.draw_caught_screen(overlay, label=self._last_caught_label)
        
        # Pause to let the student walk back to the starting line safely
    ...
    def _do_winner(self, frame, overlay, pose):
        self.ui.draw_win_screen(winner_label=self._winner_label)
    ...
    def _check_palm_yolo(self, pose_data) -> bool:
        if not pose_data or not pose_data.get("raw_data"): return False
        kpts = pose_data["raw_data"].keypoints
        if kpts is None or not hasattr(kpts, 'xy') or len(kpts.xy) == 0: return False
        
        for person in kpts.xy:
            if len(person) > 10:
                shoulder_y = min(person[5][1], person[6][1])
    ...
    def _palm_progress(self, palm_up: bool) -> float:
        if palm_up:
            if self._palm_since is None: self._palm_since = time.time()
            return min(1.0, (time.time() - self._palm_since) / self._PALM_HOLD)
        self._palm_since = None
        return 0.0

    def _handle_key(self, key):
        if key == pygame.K_ESCAPE: raise SystemExit
        if key == pygame.K_g: self._go(State.GREEN)
        if key == pygame.K_r: self._baseline_positions.clear(); self._go(State.RED)
        if key == pygame.K_w: self._trigger_winner("DEBUG WIN")
        if key == pygame.K_SPACE: self._palm_since = time.time() - self._PALM_HOLD